...
- JWT Token 包含用户 ID、账号信息，有效期 999 小时
- JWT 签名密钥：`gee`（生产环境需使用环境变量配置）
...
- **Token 有效期**：建议生产环境缩短为 24 小时，定期刷新
...
1. **生产环境使用环境变量存储密钥**：JWT 密钥不应硬编码
2. **定期更新依赖包**：修复已知安全漏洞
3. **接口限流**：防止暴力破解和 DDoS 攻击
4. **审计日志**：记录关键操作，便于追溯
...
| 版本 | 日期 | 作者 | 更新内容 |
|------|------|------|---------|
| v1.0 | 2025-10-15 | Copilot | 初始版本，完成系统设计文档 |
